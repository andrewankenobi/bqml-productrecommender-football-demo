...
-Create a view called `features-events_cumulative` defined as follows
...
FROM ( /*40% chance buys a product type 1 at the beginning if they have a fav team, and customergroup is 1*/
  ...
  FROM
    ...
    `bq-ml-football.curated.features-events_cumulative_cumulative` b
  WHERE
  ...
  FROM
    `bq-ml-football.actionable.customers`a,
    `bq-ml-football.curated.features-events_cumulative` b
  WHERE
    CustomerGroup IN (2,
      3,
      4)
...
FROM (
  ...
  END
    ...
    d.CustomerYears
  FROM
    `bq-ml-football.curated.features-events_cumulative`a
  JOIN
